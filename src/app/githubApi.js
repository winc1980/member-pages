...
async function getMonthlyCommitCounts(
  repo,
  organization,
  githubToken,
  ...
  while (true) {
    ...
    commits.forEach((commit) => {
      ...
      if (
        commitDate >= startOfPreviousMonth &&
        commitDate <= endOfPreviousMonth
      ) {
        const author = commit.author ? commit.author.login : "Unknown";
        if (!commitCounts[author]) {
          commitCounts[author] = 0;
...
export async function createMonthlyRanking() {
  ...
  for (const repo of repos) {
    const commitCounts = await getMonthlyCommitCounts(
      repo,
      organization,
      githubToken,
      startOfPreviousMonth,
...
async function getYearCommitCounts(
  repo,
  organization,
  githubToken,
  startOfYear,
...
export async function createYearlyRanking() {
  ...
  for (const repo of repos) {
    const commitCounts = await getYearCommitCounts(
      repo,
      organization,
      githubToken,
      startOfYear,
